...
public abstract class BaseBottomSheetDialogFragment<T extends BasePresenter, E extends MultiItemEntity> extends BottomSheetDialogFragment implements BaseQuickAdapter.OnItemClickListener, BaseQuickAdapter.OnItemChildClickListener {
    //日志TAG
     ...
     * 是否是下拉刷新模式
     */
    protected boolean isRefreshMode = false;
    /**
     * 分页加载当前请求页
     */
     ...
     */
    protected BaseRecyclerViewAdapter<E> mRecyclerViewAdapter;
    /**
     * 服务器返回的数据总页数
     */
  ...
  /*  @Override
     ...
     * @return
     */
    protected abstract BaseRecyclerViewAdapter<E> getRecyclerViewAdapter();
     ...
     */
    public void resetStateWhenLoadDataSuccess(@Nullable List<E> dataList) {
        if (getSmartRefreshLayout() != null) {
            if (isRefreshMode) {
...
//                                    clickEmptyErrorView();
    ...
    /**
     * 警告：该方法请勿手动调用，已经在{@link com.art.recruitment.common.baserx.RxSubscriber}中进行了调用
     * <p>
     * 开启加载进度View
     */
