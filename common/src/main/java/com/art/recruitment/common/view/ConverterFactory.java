...
public final class ConverterFactory extends Converter.Factory {
    ...
    @Override
    public Converter<ResponseBody, ? extends BaseBean> responseBodyConverter(Type type, Annotation[] annotations, Retrofit retrofit) {
        TypeAdapter<?> adapter = gson.getAdapter(TypeToken.get(type));

        return new MiaResponseConverter(gson, adapter);
    }
}
